"""Statistical inference for procurement anomaly analysis.

Implements exact hypergeometric overlap tests, percentile bootstrap confidence
intervals, Mann-Whitney U distributional comparisons, and a circularity check
excluding the audit rule that shares the new-supplier input with the model.
All analyses consume artefacts generated by preceding pipeline stages.
"""
...
def hypergeometric_triangulation_test() -> dict:
    """Test whether ML-rule overlap exceeds that expected under independence.

    The exact hypergeometric survival probability conditions on fixed marginal
    totals and is equivalent to a one-sided Fisher-type randomisation test.
    """
    ...
    # Evaluate the upper-tail probability under Hypergeometric(N, K, n).
...
# Exclude R3 because its new-supplier criterion overlaps with model input information.
...
def _hypergeometric_test_for_rule_set(val: pd.DataFrame, rule_cols: list[str], scenario: str) -> dict:
    """Calculate an exact ML-rule overlap test for a specified rule set."""
...
def hypergeometric_triangulation_circularity_check() -> pd.DataFrame:
    """Compare all-rule overlap with the R3-excluded circularity specification.

    Excluding R3 assesses whether corroboration remains after removing the
    audit criterion most mechanically related to model features.
    """
...
def _bootstrap_ci(values: np.ndarray, statistic_fn, n_bootstrap: int, random_state: int) -> tuple[float, float, float]:
    """Return a point estimate and two-sided percentile bootstrap interval."""
...
def bootstrap_anomaly_rate_by_period() -> pd.DataFrame:
    """Estimate period-specific bootstrap intervals for Isolation Forest flag rates."""
...
def bootstrap_new_supplier_rate_by_period() -> pd.DataFrame:
    """Estimate period-specific bootstrap intervals for new-supplier entry rates."""
...
def bootstrap_stl_deviation_by_period() -> pd.DataFrame:
    """Bootstrap period-level STL deviations using monthly observations.

    The percentile interval does not model temporal autocorrelation and should
    therefore be interpreted as a descriptive uncertainty approximation.
    """
...
def mann_whitney_anomaly_score_by_period() -> pd.DataFrame:
    """Compare period-specific anomaly-score distributions with Mann-Whitney tests."""
    ...
    for a, b in pairs:
        ...
        # Rank-biserial correlation quantifies the distributional separation.
        n1, n2 = len(x), len(y)
